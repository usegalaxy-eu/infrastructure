...
All instances are stored in files named `instance_<class>_<name>.tf`. 
Class can be:
* core: needed by usegalaxy.eu to run
* dedicated: dedicated to some people or a project
* extra: all other cases
Their structure is not too complex:
...
When you run `make all` terraform will sync, and if VMs need to be destroyed and
